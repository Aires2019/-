...
require("../compoents/coon.php");
 ...
 $number=@$_POST['number'];
 $price=@$_POST['price'];
 if($productid==""||$productname==""||$warehouseid==""||$prdate==""||$number==""||$price=="")
{
...
else
{
	...
	{
		$sql="
			insert into storage(productid,productname,warehouseid,prdate,number,price)
			values('$productid','$productname',$warehouseid,'$prdate',$number,$price)
		";
		 mysqli_query($link,$sql);
		 $storageid=mysqli_insert_id($link);
		 $pic="../picture/$storageid.jpg";
		 $rpic="$storageid.jpg";
		if(move_uploaded_file($upfile['tmp_name'],$pic))//移动上传文件
		{
			$sql="
				update storage set pic='$rpic' where storageid=$storageid
			";
			 mysqli_query($link,$sql);
			 echo "<script>alert('上传产品信息成功!');window.location.href='manage.php';</script>";
		}
		else
		{
				$sql="
